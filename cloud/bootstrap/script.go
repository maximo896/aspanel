...
if [ -n "$CALLBACK_URL" ] && [ -n "$TOKEN" ] && { [ -n "$AWVS_LINK" ] || [ -n "$SQLMAP_LINK" ]; }; then
  {
  ...
  ' _ "$CALLBACK_URL" "$TOKEN" "$REGION" "$ZONE" "$AWVS_LINK" "$SQLMAP_LINK" "$HEARTBEAT_INTERVAL_SEC" >> /tmp/panel-heartbeat.log 2>&1 &
fi

echo "[panel-bootstrap] finished at $(date -u +"%%Y-%%m-%%dT%%H:%%M:%%SZ")" >> "$RUN_LOG"
`, opts.AWVSInstallCommand, opts.SQLMapInstallCommand, opts.CallbackURL, opts.Token, opts.Region, opts.Zone)
}
