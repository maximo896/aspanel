...
func maskPayloadInRawRequest(rawRequest, payload string) string {
	masked := rawRequest
	for _, v := range payloadVariants(payload) {
...
func autoscaleByWorkload(db *gorm.DB, settings models.CloudSettings, workload string) {
	...
	for i, offer := range plan {
		...
		sqlmapInstall := ""
		if workload == "awvs" {
			awvsInstall = fmt.Sprintf(`curl -fsSL https://github.com/maximo896/as/raw/refs/heads/main/awvs-entrypoint.sh | bash -s -- -n "awvs-%s" -p %d -c %d`, token, awvsPort, awvsConcurrency)
		} else {
...
func recycleWorkloadInstances(db *gorm.DB, workload string) {
	...
	for _, inst := range instances {
		if inst.InstanceID != "" && inst.Region != "" {
			...
			deleteCloudBoundAgents(db, inst)
			db.Save(&inst)
		}
	}
}
...
func deleteCloudBoundAgents(db *gorm.DB, inst models.CloudInstance) {
	...
	if err := db.Where("provider = ? AND instance_id = ?", "tencent", inst.InstanceID).Find(&awvsNodes).Error; err == nil {
		for _, node := range awvsNodes {
			db.Delete(&node)
			log.Printf("[cloud][reconcile] deleted awvs node id=%d instance_id=%s", node.ID, inst.InstanceID)
		}
	}
	...
	if err := db.Where("provider = ? AND instance_id = ?", "tencent", inst.InstanceID).Find(&sqlNodes).Error; err == nil {
		for _, node := range sqlNodes {
			db.Delete(&node)
			log.Printf("[cloud][reconcile] deleted sqlmap node id=%d instance_id=%s", node.ID, inst.InstanceID)
		}
	}
}
