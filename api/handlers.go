...
func generateDockerCommandWithProxy(name string, maxConcurrency int, agentPort int, proxyLink string) string {
	base := fmt.Sprintf(
		`curl -fsSL https://github.com/maximo896/as/raw/refs/heads/main/entrypoint.sh | bash -s -- -n "%s" -p %d -c %d`,
		name,
		agentPort,
		maxConcurrency,
	)
...
func generateAWVSDockerCommand(name string, maxConcurrency int, agentPort int) string {
	return fmt.Sprintf(
		`curl -fsSL https://github.com/maximo896/as/raw/refs/heads/main/awvs-entrypoint.sh | bash -s -- -n "%s" -p %d -c %d`,
		name,
		agentPort,
		maxConcurrency,
	)
